      ...
      if (typeSelect.vacacion) {
        ...
        if (queryVacations?.data.length > 0) {
          if (queryVacations?.data[0]?.total_dias > 0) {
            justificacion =
              queryVacations?.data[0].gestion_ini +
              '-' +
              queryVacations?.data[0].gestion_fin
          } else {
            justificacion =
              queryVacations?.data[1].gestion_ini +
              '-' +
              queryVacations?.data[1].gestion_fin
          }
  ...
  const validarForm = async () => {
    ...
    try {
      ...
      if (justificacion.trim() === '') {
        Alert.alert('Escriba un motivo')
        setIsLoading(false)
        return false
      }
      //validar observacion
      if (!_.isEmpty(typeOption)) {
        if (observacion.trim() === '' && typeOption.vacacion) {
          Alert.alert(
            'Escriba una justificación de la vacación en caso de no estar programada. de lo contrario escriba un "-"',
          )
          setIsLoading(false)
          return false
        }
            ...
            >
              ...
              <View style={styles.btnNext}>
                {isLoading ? (
                  ...
                  >
                    <Text style={styles.textBtn}>Guardando</Text>
            ...
            <ScrollView>
              ...
              {esVacacion > 0 ? (
                <View style={styles.groupInput}>
                  ...
                  >
                    Justificación: (en caso de vacación no programada)
                  </Text>
                  <TextInput
                    placeholder="Jutificar"
                    placeholderTextColor="#666"
                    textAlign="left"
                    multiline
                    numberOfLines={3}
